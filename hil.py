...
fig = plt.figure()
ax = plt.axes(xlim=(0, SIM_WINDOW_WIDTH), ylim=(0, 2000))
...
def draw(fig, speed_plot, speed_data, setpoint_data):
...
while i < SIM_DURATION:
    try:
        ...
        [control_signal, setpoint] = [
            float(v) for v in line.decode("utf-8").split(";")]
        ...
        if i % SIM_REFRESH_RATE == 0:
            draw(fig, speed_plot, speed_data, setpoint_data)
